...
export const EventDetailSheet: React.FC<EventDetailSheetProps> = ({
    ...
    if (isDesktopSidebar) {
      return (
        <div className="h-full flex flex-col items-center justify-center p-8 text-center text-slate-400 bg-[#1e2330] rounded-3xl border border-slate-700/60 shadow-sm">
          <Landmark className="w-12 h-12 mb-3 text-slate-500 opacity-60" />
          <p className="text-lg font-bold text-slate-200">Select an activity</p>
  ...
  const content = (
    <div className="flex flex-col h-full overflow-y-auto space-y-5 p-5 sm:p-7 text-slate-200 custom-scrollbar">
      {/* ========================================================================= */}
      ...
      <div>
        ...
        <div className="flex items-center justify-between gap-2 mb-3">
          <div className="flex flex-wrap items-center gap-2">
            <span className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider bg-[#282e3e] text-slate-200 border border-slate-700">
              {getTypeIcon(event.type)}
              <span>{event.type}</span>
            </span>

            {event.isFixed ? (
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-amber-500/20 text-amber-200 border border-amber-500/30">
                <Lock className="w-3.5 h-3.5 text-amber-400" /> Fixed Slot
              </span>
            ) : (
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium bg-[#282e3e] text-slate-400 border border-slate-700">
          ...
          {!isDesktopSidebar && (
            <button
              onClick={onClose}
              className="p-2 rounded-full bg-[#282e3e] text-slate-400 hover:text-white hover:bg-[#31384b] transition border border-slate-700"
              aria-label="Close"
            >
              <X className="w-5 h-5" />
            </button>
        ...
        {/* Time & Duration Bar */}
        <div className="flex flex-wrap items-center gap-2 mt-2.5 text-sm sm:text-base font-semibold text-slate-300">
          <span className="flex items-center gap-2 text-white font-bold font-mono">
            <Clock className="w-4 h-4 text-indigo-400 shrink-0" />
            {formatEventTime(event.startTime)} – {formatEventTime(event.endTime)}
          </span>
          <span className="text-slate-500">•</span>
          <span className="px-2.5 py-0.5 rounded-lg bg-[#282e3e] text-slate-200 font-mono text-xs font-bold border border-slate-700">
        ...
        {event.what && (
          <p className="text-sm sm:text-base text-slate-300 leading-relaxed font-normal mt-3 pt-3 border-t border-slate-700/60">
            {event.what}
          </p>
        )}
      </div>
          ...
          <button
            ...
            className={`flex items-center justify-center gap-2 py-3 px-4 rounded-2xl text-xs sm:text-sm font-bold border transition ${
              event.status === 'done'
                ...
                : 'bg-[#282e3e] border-slate-700 text-slate-300 hover:bg-[#31384b] hover:text-white'
            }`}
          >
      ...
      <div className="bg-[#242938] rounded-2xl p-5 sm:p-6 border border-slate-700/70 shadow-sm space-y-4">
        {/* Section Header */}
        <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-indigo-300 border-b border-slate-700/60 pb-3">
        ...
        {event.food && (
          <div className="p-3.5 sm:p-4 rounded-xl bg-rose-950/30 border border-rose-500/30 space-y-1.5">
      ...
      <div className="bg-[#242938] rounded-2xl p-5 sm:p-6 border border-slate-700/70 shadow-sm space-y-3.5">
        {/* Section Header */}
        <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-emerald-400 border-b border-slate-700/60 pb-3">
        ...
        {(event.ticket || event.reservation) && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 pt-1">
            {event.ticket && (
              <div className="p-3.5 rounded-xl bg-[#1d222e] border border-slate-750">
        ...
        {event.notes && (
          <div className="p-3.5 rounded-xl bg-[#1d222e] border border-slate-750 space-y-1">
        ...
        {/* Exact Address */}
        {event.location && (
          <div className="p-3.5 rounded-xl bg-[#1d222e] border border-slate-750 flex items-start gap-2.5">
            <MapPin className="w-4 h-4 text-sky-400 shrink-0 mt-0.5" />
            <div className="text-xs sm:text-sm text-slate-300 leading-relaxed">
              <span className="font-bold text-slate-400 uppercase tracking-wider text-[11px] block mb-0.5">
                Exact Address
              </span>
            ...
            <div className="flex flex-wrap gap-2">
              {event.links.map((link, idx) => (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-2 px-3.5 py-2 rounded-xl bg-[#282e3e] hover:bg-[#31384b] border border-slate-700 text-xs font-semibold text-slate-300 hover:text-white transition group"
                >
                  {getLinkIcon(link)}
