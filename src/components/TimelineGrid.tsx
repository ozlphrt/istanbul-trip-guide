...
export const TimelineGrid: React.FC<TimelineGridProps> = ({
  ...
  return (
    <div className="bg-[#1f2431] rounded-2xl sm:rounded-3xl border border-slate-700/70 shadow-inner p-2 sm:p-4 mb-8 select-none">
      {/* Scrollable Timeline Canvas (100% Full Width — Concept 5) */}
      <div
        className="relative w-full"
        style={{ height: `${TIMELINE_TOTAL_HEIGHT_PX}px` }}
      >
        {/* Horizontal 15-Minute Grid Lines & Subtle Hour Stamp Markers */}
        {hours.map((hour) => {
          ...
          return (
            <React.Fragment key={`grid-${hour}`}>
              {/* Major Hour Solid Line (:00) with Subtle Hour Stamps on Edges */}
              <div
                style={{ top: `${topOffsetPx}px` }}
                className="absolute inset-x-0 border-t border-slate-700/80 pointer-events-none flex items-center justify-between px-2 pt-0.5"
              >
                <span className="text-[10px] sm:text-xs font-mono font-bold text-slate-500/40 select-none">{displayHour}</span>
                <span className="text-[10px] sm:text-xs font-mono font-bold text-slate-500/40 select-none">{displayHour}</span>
              </div>
              ...
              {hour < TIMELINE_END_HOUR && (
                <>
                  ...
                  <div
                    ...
                    className="absolute inset-x-0 border-t border-dotted border-slate-700/30 pointer-events-none"
                  />

                  {/* 30-Minute Dashed Line (:30) */}
                  <div
                    style={{ top: `${top30Px}px` }}
                    className="absolute inset-x-0 border-t border-dashed border-slate-700/50 pointer-events-none"
                  />
        ...
        {/* Render 100% Full-Width Event Blocks */}
